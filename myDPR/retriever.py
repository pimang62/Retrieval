...
class DPRRetriever:
    def __init__(self, model, valid_dataset, index, article_path, mode, val_batch_size: int = 64, device='cuda:1' if torch.cuda.is_available() else "cpu"):
...
if __name__ == "__main__":
    ...
    model.load("model/qa_model.pt")  # "model/kobert_biencoder.pt" / "model/my_model.pt"
    model.eval()
    valid_dataset = Dataset(data_path="dataset/baemin.json",
                            passages_dir="baemin_passages_answer",
                            title_index_map_path="baemin_title_passage_map_answer.p",
                            passages_type="answer")
    
    index = DenseFlatIndexer()
    index.deserialize(path="index", file_name="baemin")

    retriever = DPRRetriever(model=model, 
                             valid_dataset=valid_dataset, 
                             article_path='article/baemin.p',
                             index=index,
                             mode="article")
