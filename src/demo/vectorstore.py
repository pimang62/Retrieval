...
from chunker.data_chunk import ChunkerFactory
from embeddings.huggingface import TransformersEmbedding
from embeddings.openai_api import OpenAIEmbedding

import faiss
from faiss import write_index, read_index
import numpy as np
from utils.convert import convert_to_numpy  # List[List[float]] -> np.ndarray
...
class VectorStore:
    def __init__(self, database_name, embedding_model_name_or_path, chunk_size, chunk_overlap):  # <- fname X
        ...
        self.vectorstore_path = os.path.join(os.pardir, "vectorstore")
        os.makedirs(self.vectorstore_path, exist_ok=True)

        #### umap_models 저장 디렉토리 생성
        self.umap_dir_path = os.path.join(os.pardir, "umap_models")
